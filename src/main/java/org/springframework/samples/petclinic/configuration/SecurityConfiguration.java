...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/resources/**", "/webjars/**", "/h2-console/**").permitAll().antMatchers(HttpMethod.GET, "/", "/oups").permitAll().antMatchers("/users/new").permitAll().antMatchers("/admin/**").hasAnyAuthority("admin")
			...
			.antMatchers("/jams/*/teams/new", "/jams/*/teams/*/edit").authenticated().antMatchers("/jams/*/teams/*").permitAll().antMatchers("/jams/*/teams/*/invitationList", "/invitations/*/delete").permitAll().antMatchers("/invitations").authenticated()
